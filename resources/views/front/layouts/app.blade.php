...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
  <head>
    ...
    <meta
      property="og:image"
      content="{{ is_logo($page_image ?? '') }}?auto=format&fit=max&w=1200"
    />
    ...
    <meta
      ...
      content="w5e39xmuhdt35pjpezg5pkif7f501x"
    />
    ...
    />
    <link
      href="{{ asset('front/lib/animate/animate.min.css') }}"
      rel="stylesheet"
    />
    <link
    ...
    <link
      href="https://cdnjs.cloudflare.com/ajax/libs/jqueryui/1.13.3/themes/smoothness/jquery-ui.min.css"
      rel="stylesheet"
    />
    @stack('style')
  </head>
  <body>
    <div class="container-fluid bg-white p-0">
      ...
      @include('front.partials.slider') @endif
      <main>@yield('content')</main>
      @if (Route::is('beranda'))@include('front.partials.testimonial') @include('front.partials.mitra')@endif {{-- Footer --}} @include('front.partials.footer')
...
document.getElementById('btnDirections').addEventListener('click', function() {
    ...
    if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(
            ...
            function(error) {
                // fallback jika user menolak lokasi
                const mapsUrl = `https://www.google.com/maps/dir/?api=1&origin=My+Location&destination=${destLat},${destLng}&travelmode=driving`;
                window.open(mapsUrl, '_blank');
            }
        );
    } else {
    ...
    >
      <div
        ...
        aria-live="assertive"
        aria-atomic="true"
        style="display: none; min-width: 250px; max-width: 350px;"
      >
        <div class="d-flex">
          <div
            class="toast-body text-center fw-semibold"
            id="centerToastBody"
          ></div>
          <button
            type="button"
