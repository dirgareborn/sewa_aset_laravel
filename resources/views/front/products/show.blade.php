        ...
        <div class="container">
            <div class="card rounded-0 border-0 animated fadeIn">
                <div class="row g-5 wow fadeIn" data-wow-delay="0.1s">
                    ...
                    <div class="col-md-6 preview border-0 shadow-sm">
                        <div class="preview-pic tab-content" id="myTabContent">
                            ...
                            @foreach ($product['images'] as $key => $image)
                                <div class="tab-pane fade @if ($key == 0) show active @endif"
                                    ...
                                    <img
                                        @if ($key == 0) src="{{ is_product($product['product_image']) }}"
                    @else
                        src="{{ is_slide($image['image']) }}" @endif>
                            ...
                            @foreach ($product['images'] as $key => $image)
                                <li class="@if ($key == 0) active @endif">
                                    <a data-bs-target="#pic-{{ $key }}" data-bs-toggle="tab">
                                        ...
                                        <img class="img-thumbnail thumbnail shadow-sm my-2"
                                            @if ($key == 0) src="{{ is_product($product['product_image']) }}"
                        @else
                            src="{{ is_slide($image['image']) }}" @endif
                    ...
                    <div class="details col-md-6">
                        ...
                        <div class="rating" data-product-id="{{ $product->id }}">
                            <div class="stars">
                                @for ($i = 1; $i <= 5; $i++)
                                    <span class="fa fa-star {{ $i <= round($average_rating ?? 0) ? 'checked' : '' }}"
                                        data-value="{{ $i }}"></span>
                                @endfor
                            </div>
                            <span class="review-no">{{ $total_reviews ?? 0 }} reviews</span>
                            <strong>{{ round((($average_rating ?? 0) / 5) * 100) }}%</strong>
                        </div>
                        ...
                        <!-- Toast -->
                        <div id="guestToast" class="toast position-fixed top-50 start-50 translate-middle" role="alert"
                            data-bs-delay="3000">
                        ...
                        <ul class="list-unstyled">
                            @forelse($product['product_facility'] ?? []  as $facility)
                                <li class="">
                        ...
                        <form id="addCart" name="addCart" action="javascript:;" method="POST">
                            ...
                            <div class="row g-2">
                                <div class="col-md-10">
                                    <div class="row g-2">
                                        <div class="col-md-4">
                                            <input id="start" name="start" class="date form-control py-3"
                                                placeholder="Mulai Tanggal ">
                                        </div>
                                        <div class="col-md-4">
                                            <input id="end" name="end" class="date form-control  py-3"
            ...
            async function submitRating(rating, review) {
                try {
                    ...
                    if (data.success) {
                        ...
                        document.querySelector('.vote strong').textContent =
                            `${Math.round(data.average_rating/5*100)}%`;
