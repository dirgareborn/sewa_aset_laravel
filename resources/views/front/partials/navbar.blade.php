<div class="container-fluid nav-bar bg-dark">
    <nav class="navbar navbar-expand-lg navbar-dark py-0 px-4 m-0">
        ...
        <div class="d-flex align-items-center ms-auto d-lg-none">
            ...
            <button class="navbar-toggler custom-toggler text-orange" type="button" data-bs-toggle="collapse" data-bs-target="#navbarCollapse">
        ...
        <div class="collapse navbar-collapse" id="navbarCollapse">
            <div class="navbar-nav ms-auto">
                ...
                <div class="nav-item dropdown">
                    <a href="#" class="nav-link dropdown-toggle {{ request()->is('visi-misi', 'struktur-organisasi') ? 'active' : '' }}" data-bs-toggle="dropdown">Tentang BPB</a>
                    <ul class="dropdown-menu bg-dark rounded-0 m-0">
                        <li><a href="{{ url('visi-misi') }}" class="dropdown-item">Visi Misi</a></li>
                        <li><a href="{{ url('struktur-organisasi') }}" class="dropdown-item">Struktur Organisasi</a></li>
                    </ul>
                </div>

                <!-- Unit Usaha Dropdown Multi-level -->
                <div class="nav-item dropdown">
                    <a href="#" class="nav-link dropdown-toggle {{ request()->is('kategori') || request()->is('kategori/*') ? 'active' : '' }}" data-bs-toggle="dropdown">Unit Usaha</a>
                    <ul class="dropdown-menu bg-dark rounded-0 m-0">
                        @foreach ($MenuCategories->where('parent_id', 0) as $parent)
                            @php $children = $MenuCategories->where('parent_id', $parent->id); @endphp
                            <li class="dropdown-submenu">
                                <a class="dropdown-item dropdown-toggle" href="{{ $children->count() ? '#' : url('kategori/' . $parent->url) }}">
                                    {{ ucfirst($parent->category_name) }}
                                </a>
                                @if ($children->count())
                                    <ul class="dropdown-menu bg-dark">
                                        @foreach ($children as $child)
                                            <li><a class="dropdown-item" href="{{ url('kategori/' . $child->url) }}">{{ ucfirst($child->category_name) }}</a></li>
                                        @endforeach
                                    </ul>
                                @endif
                            </li>
                ...
                <!-- Menu Baru -->
                <a href="{{ route('kerjasama') }}" class="nav-item nav-link {{ request()->is('kerjasama') ? 'active' : '' }}">Kerja Sama</a>
                <a href="{{ route('dokumen.index') }}" class="nav-item nav-link {{ request()->is('dokumen') ? 'active' : '' }}">Dokumen</a>
                ...
                @if (Auth::check())
                    <div class="nav-item dropdown">
                        <a class="nav-link dropdown-toggle d-flex align-items-center" data-bs-toggle="dropdown" href="#" id="navbarDropdown" role="button">
                            <img src="{{ is_user(Auth::user()->image) }}" class="rounded-circle" height="25" alt="user image" loading="lazy">
                        </a>
                ...
                @if (Auth::check())
                    <div class="nav-item nav-link dropdown" id="appendMiniCartItems">
                        <a class="text-reset me-3 d-none d-sm-inline" href="{{ url('cart') }}">
                            ...
                            <span class="badge rounded-pill badge-notification d-none d-sm-inline bg-danger">{{ $totalCartItems }}</span>
                        </a>
    ...
    <div id="searchOverlay" class="search-overlay">
        <div class="search-box">
            <form action="{{ route('global.search') }}" method="get" class="d-flex justify-content-center align-items-center w-100">
